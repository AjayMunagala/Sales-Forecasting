...
def generate_sales_forecast():
    ...
    # Add labels to highlight the axes
    for i, (date, price) in enumerate(zip(new_dates, new_prices)):
        plt.annotate(f'{price:.2f}', (date, price), xytext=(0, 10), textcoords='offset points',
...
def generate_new_dates(start_date, periodicity, periodic_number):
    new_dates = pd.date_range(start=start_date, periods=periodic_number + 1, freq=periodicity)[1:]
    return new_dates.strftime('%Y-%m-%d').tolist()
